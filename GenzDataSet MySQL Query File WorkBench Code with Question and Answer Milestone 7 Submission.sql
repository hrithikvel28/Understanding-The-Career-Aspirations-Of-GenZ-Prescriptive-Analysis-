...
select Gender,avg(CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(ExpectedSalary3Years,'to',1),'k',1)AS signed)) 
AS extracted_value
from personalized_info p
join mission_aspirations mis on p.ResponseID = mis.ResponseID
group by Gender;
...
-- 18. What is the Average Higher Bar salary expectations at 5 year mark for state in India each gender and each state in India --
WITH state AS (
SELECT ResponseID, Gender,
    CASE
        WHEN SUBSTRING(zipcode, 1, 2) BETWEEN '11' AND '11' THEN 'Delhi'
